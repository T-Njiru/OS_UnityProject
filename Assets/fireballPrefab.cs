...
public class Projectile : MonoBehaviour
{
    ...
    [SerializeField] private float travelDistance = 10f;  // Define the distance the fireball must travel before exploding
    ...
    private void Update()
    {
        ...
        // Check if fireball has traveled the required distance
        if (Vector3.Distance(startPosition, transform.position) >= travelDistance)
        {
            // Trigger the explosion if the fireball has traveled enough distance
            Explode();
        }
    ...
    {
        if (!hit)  // Ensure explosion only happens once
        {
            hit = true;
            anim.SetTrigger("explode");
            Debug.Log("Explosion animation triggered due to travel distance!");

            // Apply damage and deactivate after explosion
            Collider2D[] enemies = Physics2D.OverlapCircleAll(transform.position, 1f);  // Adjust the radius as needed
            foreach (var enemy in enemies)
            {
                if (enemy.CompareTag("Enemy"))
                {
                    enemy.GetComponent<Enemy>().TakeDamage(damage);  // Apply damage to the enemy
                }
            }
